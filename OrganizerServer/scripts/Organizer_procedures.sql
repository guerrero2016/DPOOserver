...
CREATE PROCEDURE Organizer.requestTop10 ()
	BEGIN
		(SELECT nom_usuari, COUNT(*) as tasques_per_fer
		 FROM Tasca_Usuari as tu JOIN Tasca as t ON t.id_tasca = tu.id_tasca
		 WHERE data_done IS null
		 GROUP BY nom_usuari
		 ORDER BY tasques_per_fer DESC)
		UNION ALL
		(SELECT nom_usuari, 0 as tasques_per_fer
		 FROM Tasca_Usuari as tu JOIN Tasca as t ON t.id_tasca = tu.id_tasca
		 WHERE data_done IS NOT null
		 GROUP BY nom_usuari
		 ORDER BY tasques_per_fer ASC)
		LIMIT 10;
	END $$
DELIMITER ;
